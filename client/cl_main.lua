...
local CreateDropOff = function()
	...
	exports['okokNotify']:Alert("OXY RUNS", "BUYER MARKED ON GPS", 3000, 'success')
	dropOffCount += 1
	...
	dropOffArea:onPlayerInOut(function(isPointInside, point)
		if isPointInside then
			...
			--	TriggerEvent('qb-phone:client:CustomNotification', 'CURRENT', "Make the delivery..", 'fas fa-capsules', '#3480eb', 8000)
				exports['okokNotify']:Alert("OXY RUNS", "EXIT  CAR TO MAKE DELIVERY!", 4000, 'success')
...
local StartOxyrun = function()
	...
	started = true
	--TriggerEvent('qb-phone:client:CustomNotification', 'CURRENT', "WE WILL SEND NEW LOCATION", 'fas fa-capsules', '#3480eb', 8000)
	exports['okokNotify']:Alert("OXY RUNS", "WAIT FOR BUYERS!", 3000, 'success')
	while started do
		Wait(4000)
...
RegisterNetEvent('qb-oxyruns:client:DeliverOxy', function()
	...
	if #(GetEntityCoords(ped) - GetEntityCoords(oxyPed)) < 5.0 then
		...
		if dropOffCount == Config.RunAmount then
			...
			exports['okokNotify']:Alert("OXY RUNS", "JOB COMPLETED!", 3000, 'success')
			started = false
			dropOffCount = 0
		else
			...
			exports['okokNotify']:Alert("OXY RUNS", "WAIT FOR BUYER!", 3000, 'success')
		end
		DeleteOxyped()
		hasDropOff = false
		madeDeal = false
		...
		options = {
			{
				...
				icon = 'fas fa-capsules',
				label = ' OXY ['..Config.StartOxyPayment..']',
			}
		},
		distance = 2.0
	})
